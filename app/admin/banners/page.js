...
export default function BannersAdmin(){
  ...
  function openAdd(){setForm({id:uuidv4(),title:'',subtitle:'',buttonText:'',buttonUrl:'',imageUrl:'',bgColor:'#1e3a5f',textColor:'#ffffff',isActive:true,order:items.length,startDate:'',endDate:''});setModal('add');}
  ...
  return(
    <div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b">
              <h2 className="font-bold text-[#1e3a5f]">{modal==='add'?'Tambah Banner':'Edit Banner'}</h2>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div><Label className="text-xs font-semibold mb-1 block">Warna BG</Label><Input type="color" value={form.bgColor||'#1e3a5f'} onChange={e=>setForm(p=>({...p,bgColor:e.target.value}))} className="h-10 px-2"/></div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-extrabold text-[#1e3a5f]">Banner & Slider</h1><p className="text-gray-500 text-sm mt-1">{items.length} banner</p></div>
        <Button className="bg-[#1e3a5f] hover:bg-[#2d5a8e] text-white" onClick={openAdd}><Plus className="w-4 h-4 mr-1"/>Tambah Banner</Button>
      </div>
          ...
          {items.sort((a,b)=>a.order-b.order).map(item=>(
            <div key={item.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${!item.isActive?'opacity-60 border-red-200':'border-gray-100'}`}>
              <div className="flex gap-4 p-4">
                <div className="w-40 h-24 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100" style={{background:item.bgColor||'#1e3a5f'}}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      {item.buttonText&&<a href={item.buttonUrl||'#'} className="text-xs text-[#c9a84c] font-semibold mt-1 inline-block">{item.buttonText} →</a>}
                    </div>
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button onClick={()=>toggleActive(item)} className="p-1.5 text-gray-400 hover:text-[#1e3a5f] rounded-lg">{item.isActive?<EyeOff className="w-4 h-4"/>:<Eye className="w-4 h-4"/>}</button>
                      <button onClick={()=>openEdit(item)} className="p-1.5 text-gray-400 hover:text-[#1e3a5f] rounded-lg"><Edit2 className="w-4 h-4"/></button>
