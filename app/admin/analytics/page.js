  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-[#1e3a5f]">Statistik Pengunjung</h1>
          <p className="text-gray-500 text-sm mt-1">Analisis kunjungan website</p>
        </div>
        <div className="flex gap-2">
          {[7, 14, 30, 90].map(d => (
            <button key={d} onClick={() => setDays(d)}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${ days === d ? 'bg-[#1e3a5f] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }`}>
              {d} Hari
            </button>
          ))}
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <p className="text-3xl font-extrabold text-[#1e3a5f]">{data.total.toLocaleString()}</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <p className="text-3xl font-extrabold text-[#1e3a5f]">{data.dailyData.length ? Math.round(data.total / data.dailyData.length).toLocaleString() : 0}</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <p className="text-lg font-extrabold text-[#1e3a5f] truncate">{data.topPages[0]?.path || '-'}</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
            <h2 className="font-bold text-[#1e3a5f] mb-4 flex items-center gap-2"><BarChart2 className="w-5 h-5"/>Tren Kunjungan Harian</h2>
              ...
              <div className="flex items-end gap-1 h-48 overflow-x-auto pb-2">
                {data.dailyData.slice(-30).map(d => (
                  <div key={d.date} className="flex flex-col items-center gap-1 min-w-[28px] flex-1">
                    <div className="w-full rounded-t-md bg-[#1e3a5f] transition-all" style={{ height: `${(d.views / maxViews) * 160}px`, minHeight: d.views > 0 ? 4 : 0 }} title={`${d.date}: ${d.views} kunjungan`} />
          ...
          {/* Top pages */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h2 className="font-bold text-[#1e3a5f] mb-4">Halaman Paling Banyak Dikunjungi</h2>
              ...
              <div className="space-y-3">
                {data.topPages.map((p, i) => (
                  <div key={p.path} className="flex items-center gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#1e3a5f] text-white text-xs flex items-center justify-center font-bold flex-shrink-0">{i+1}</span>
                    <span className="flex-1 text-sm text-gray-700 truncate font-mono">{p.path}</span>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <div className="w-24 bg-gray-100 rounded-full h-2">
                        <div className="bg-[#c9a84c] h-2 rounded-full" style={{ width: `${(p.views / data.topPages[0].views) * 100}%` }} />
                      </div>
                      <span className="text-sm font-semibold text-[#1e3a5f] w-12 text-right">{p.views.toLocaleString()}</span>
                    </div>
                  </div>
                ))}
              </div>
