...
export default function AnnouncementsAdmin() {
  ...
  const [form, setForm] = useState({ title: '', content: '', publishDate: '', isActive: true });
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/announcements?page=${page}&limit=10&search=${encodeURIComponent(search)}`);
  ...
  function openCreate() {
    setForm({ title: '', content: '', publishDate: new Date().toISOString().split('T')[0], isActive: true });
    setModal('create');
  }

  function openEdit(item) {
    setForm({ title: item.title, content: item.content, publishDate: item.publishDate || '', isActive: item.isActive ?? true });
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(isEdit ? `/api/announcements/${modal.id}` : '/api/announcements', {
        method: isEdit ? 'PUT' : 'POST', headers, body: JSON.stringify(form)
      });
      if (!res.ok) throw new Error();
      showToast(isEdit ? 'Pengumuman diperbarui' : 'Pengumuman ditambahkan');
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-4">
        <div className="relative max-w-sm">
          ...
          <Input placeholder="Cari pengumuman..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="pl-9" />
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Pengumuman</th>
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">Tanggal</th>
            ...
            {loading ? (
              [1,2,3].map(i => (
                <tr key={i} className="animate-pulse">
                  <td className="px-4 py-4"><div className="space-y-2"><div className="h-4 bg-gray-200 rounded w-3/4" /><div className="h-3 bg-gray-200 rounded w-1/2" /></div></td>
      ...
      <Dialog open={!!modal} onOpenChange={v => !v && setModal(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{modal?.type === 'edit' ? 'Edit Pengumuman' : 'Tambah Pengumuman'}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div>
              <Label className="text-sm font-medium mb-1.5 block">Judul <span className="text-red-500">*</span></Label>
              <Input placeholder="Judul pengumuman" value={form.title} onChange={e => setForm(f => ({...f, title: e.target.value}))} />
            </div>
            <div>
