...
export default function DokumenPage() {
  ...
  async function handleDownload(item) {
    if (!item.fileUrl) { alert('File belum tersedia'); return; }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-[#1e3a5f] py-16">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-4xl font-extrabold text-white mb-3">{lang === 'id' ? 'Dokumen & Peraturan' : 'Documents & Regulations'}</h1>
          <p className="text-white/70 max-w-xl mx-auto">{lang === 'id' ? 'Unduh dokumen, formulir, dan peraturan resmi Pengadilan Agama Penajam' : 'Download official documents, forms and regulations'}</p>
        </div>
      </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {items.map(item => (
              <div key={item.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-start gap-4 hover:shadow-md transition-shadow">
                ...
                <button onClick={() => handleDownload(item)} className={`flex-shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-all ${item.fileUrl?'bg-[#1e3a5f] hover:bg-[#2d5a8e] text-white':'bg-gray-100 text-gray-400 cursor-not-allowed'}`} disabled={!item.fileUrl}>
