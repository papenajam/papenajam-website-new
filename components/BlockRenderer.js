...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   Used by: Page Builder canvas, /halaman/[slug], DynamicHomepage
   ───────────────────────────────────────────────────────────────────────────── */
  ...
  return (
    ...
    >
      <div className="relative z-10 container mx-auto px-6 max-w-4xl">
        {s.tag && <span className="inline-block text-xs font-bold tracking-widest uppercase text-[#d4a017] bg-white/10 px-4 py-1.5 rounded-full mb-5">{s.tag}</span>}
        <h1 className="text-3xl md:text-5xl font-extrabold text-white leading-tight mb-4 drop-shadow-md">{s.title || 'Judul Hero'}</h1>
        {s.subtitle && <p className="text-white/80 text-lg md:text-xl max-w-2xl mx-auto mb-8 leading-relaxed">{s.subtitle}</p>}
        {s.buttonText && s.buttonLink && (
          <a href={s.buttonLink} className="inline-block bg-[#d4a017] hover:bg-[#b88010] text-white font-bold px-8 py-3.5 rounded-xl text-base transition-all hover:shadow-lg hover:-translate-y-0.5">{s.buttonText}</a>
        )}
      </div>
    </section>
  );
  ...
  return (
    <section className="py-12 bg-white">
      <div className="container mx-auto px-4 max-w-3xl">
        {s.title && <h2 className="text-2xl md:text-3xl font-bold text-[#1b5e20] mb-5">{s.title}</h2>}
        <div className="prose prose-lg prose-headings:text-[#1b5e20] prose-a:text-[#d4a017] max-w-none text-gray-700 leading-relaxed"
    ...
    <section className="py-14 bg-gray-50">
      <div className="container mx-auto px-4">
        ...
        <div className={`grid grid-cols-1 ${cols} gap-6 max-w-5xl mx-auto`}>
          ...
          {!(s.items||[]).length && <div className="col-span-3 py-10 text-center text-gray-400 border-2 border-dashed rounded-2xl">Tambahkan kartu di pengaturan</div>}
        </div>
      </div>
    </section>
  );
...
function StatsBlock({ settings: s }) {
  ...
  return (
    <section className="py-14" style={{ background: s.bgColor || '#1b5e20' }}>
      <div className="container mx-auto px-4">
        {s.title && <h2 className="text-xl font-bold text-white text-center mb-8">{s.title}</h2>}
        <div className={`grid grid-cols-2 md:grid-cols-${count} gap-6 max-w-4xl mx-auto`}>
          {(s.items||[]).map((item) => (
            <div key={item.id} className="text-center">
              <div className="text-3xl md:text-5xl font-extrabold text-[#d4a017] mb-2">{item.number}</div>
              <div className="text-white/70 text-sm font-medium">{item.label}</div>
            </div>
          ))}
          {!(s.items||[]).length && <div className="col-span-4 py-8 text-center text-white/40">Tambahkan statistik di pengaturan</div>}
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="py-16" style={{ background: s.bgColor || '#1b5e20' }}>
      <div className="container mx-auto px-4 text-center text-white">
        {s.title && <h2 className="text-2xl md:text-4xl font-extrabold mb-4">{s.title}</h2>}
        {s.subtitle && <p className="text-white/75 mb-8 text-lg max-w-xl mx-auto">{s.subtitle}</p>}
        {s.buttonText && s.buttonLink && (
...
function GalleryBlock({ settings: s }) {
  ...
  return (
    <section className="py-10 bg-white">
      <div className="container mx-auto px-4">
        {s.title && <h2 className="text-2xl font-bold text-[#1b5e20] text-center mb-8">{s.title}</h2>}
        <div className={`grid ${cols} gap-3 md:gap-4`}>
          ...
          {!(s.images||[]).length && <div className="col-span-3 py-12 text-center text-gray-400 border-2 border-dashed rounded-2xl">Tambahkan gambar di pengaturan</div>}
        </div>
      </div>
    </section>
  );
    ...
    <section className="py-14 bg-white">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <div className="space-y-2">
          {(s.items||[]).map((item,i) => (
            <div key={item.id||i} className={`border rounded-2xl overflow-hidden transition-all ${openIdx===i ? 'border-[#1b5e20]/40 shadow-sm' : 'border-gray-200'}`}>
              <button onClick={() => setOpenIdx(openIdx===i ? null : i)}
                ...
                <span className={`font-semibold text-base ${openIdx===i ? 'text-[#1b5e20]' : 'text-gray-800'}`}>{item.question||'Pertanyaan...'}</span>
          ...
          {!(s.items||[]).length && <div className="py-12 text-center text-gray-400 border-2 border-dashed rounded-2xl">Tambahkan item accordion di pengaturan</div>}
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="py-14 bg-white">
      <div className="container mx-auto px-4 max-w-4xl">
        {s.title && <h2 className="text-2xl md:text-3xl font-bold text-[#1b5e20] text-center mb-6">{s.title}</h2>}
        {tabs.length > 0 ? (
          <>
...
function MapBlock({ settings: s }) {
  return (
    <section className="py-10 bg-white">
      <div className="container mx-auto px-4">
        {s.title && <h2 className="text-2xl font-bold text-[#1b5e20] text-center mb-5">📍 {s.title}</h2>}
        {s.embedUrl ? (
...
export { HeroBlock, TextBlock, ImageBlock, CardGridBlock, StatsBlock, CtaBlock, GalleryBlock, AccordionBlock, TabsBlock, MapBlock, CountdownBlock };
