// Response serializer for PostgreSQL/Prisma rows.
//
// It formats calendar dates and timestamps for the API, preserves JSONB
// structures, converts BigInt values safely, and never exposes password or
// internal `_id` fields.
 ...
 * pg adapter does not currently emit Date inside JsonB, but previous datastore used to
...
function coerceJson(value) {
  ...
  if (typeof value === 'object') {
    ...
    for (const [k, v] of Object.entries(value)) {
      ...
      // internal fields. The established API docs never embedded passwords in
...
/**
 * Serialize a single Prisma row to the established API wire shape.
 *
 * @param {string} model - Prisma model name (e.g. `'News'`) OR established API
...
export function serializeRecord(model, record) {
  ...
  for (const [key, raw] of Object.entries(record)) {
    // Always drop internal fields and the password hash. `_id` is dropped per
