// Prisma Client singleton for the MongoDB -> PostgreSQL migration.
//
// Plan section 7 / Task 5 binding contract (updated for Next.js build safety):
//   - Load dotenv so runtime scripts and Next.js pick up `.env`.
//   - Lazy-create the PrismaPg adapter + PrismaClient on first access so
//     `next build` page-data collection does NOT require DATABASE_URL at
//     module-evaluation time (handlers import this module at build).
//   - Cache the client on `globalThis` in development so Next.js HMR does
//     not open a new pool on every hot reload.
//   - Do NOT call `$disconnect()` per request; the pool lives for the
//     process lifetime (plan line 370).
//   - Query logging is opt-in via env and must not print secrets/PII.
//   - Pass `schema` to PrismaPg as a second constructor option (adapter-pg
//     does not accept `?schema=` on the connection string; that param is
//     Prisma-engine specific and is rejected by node-postgres / psql).
//
// Import path: the generated client lives at `lib/generated/prisma`
// (per schema.prisma `generator client.output`; see deviation note there).
// Application source stays JavaScript; the `.ts` specifier matches the
// generator's importFileExtension so Next.js can resolve the client.
